...
# default robot capability (tune for your robot)
robot_max_speed = 0.5    # m/s (adjust)
robot_max_accel = 0.5    # m/s^2 (adjust)
...
class BallEKF:
    """
    Simple 6-state EKF for ballistic motion:
      x = [X, Y, Z, Vx, Vy, Vz]
    Process: constant velocity with gravity applied to Vz.
    """
    def __init__(self, pos_sigma=0.02, vel_sigma=1.0, accel_sigma=7.0):
        self.x = None   # 6-vector
        self.P = None   # 6x6 covariance
        self.last_ts = None
    ...
    def update(self, meas_pos, meas_R=None, meas_ts=None):
        """
        meas_pos: 3-vector world pos
        meas_R: 3x3 covariance matrix (optional)
        meas_ts: timestamp of measurement (epoch seconds) optional
        returns True if update applied, False if rejected (outlier) or not ready
        """
        ...
        # if measurement timestamp newer than last_ts, predict forward
    ...
    def predict_future(self, tau):
        """
        Return predicted state at tau seconds ahead, without modifying filter.
        Returns 6-vector: [x,y,z,vx,vy,vz]
        """
        if self.x is None:
            return None
        s = self.x.copy()
...
def compute_robot_center_from_tags(tag_info):
    """
    Given tag_info dict with possible entries 'tag4' and 'tag5', where each tag entry
    provides a 'world_pos' (3-vector) and optionally 'y_axis_unit' (unit vector),
    compute robot center as:
      - if both present: midpoint of their world_pos
      - if only tag4: shift tag4 world_pos along its y_axis by -0.096 m
      - if only tag5: shift tag5 world_pos along its y_axis by +0.096 m
    """
...
def append_prediction_log(predicted_pos, predicted_t, current_time, robot_pos, v_req=None, can_reach=None):
    entry = {
                        ...
                        try:
                            ...
                            for i,idarr in enumerate(ids):
                                ...
                                if tid in (4,5):
                                    ...
                                    # compute y_axis unit if possible: approximate from corner order
                                    # corners order: [-half,+half], [+half,+half], [+half,-half], [-half,-half]
                                    # take vector from corner0->corner1 in world to get tag local +X, and corner0->corner3 for -Y; derive +Y.
                                    try:
...
shared_robot_poses = {}  # cam_name/tagid -> latest robot tag poses in world coords
last_triangulated_ball = None

# EKF instance (single ball track for now)
ball_ekf = BallEKF(pos_sigma=0.03, vel_sigma=1.0, accel_sigma=7.0)
    ...
    while True:
        ...
        if ready:
            ...
            if len(tri_results) > 0:
                # --- EKF integration: use best triangulation result
                best = tri_results[0]
                Xw = best['pt']  # 3-vector world coordinates
                last_triangulated_ball = {'pos': Xw, 'err': best['reproj_err'], 'ts': time.time(), 'pair': best['pair'], 'depths':best['depths']}

                meas_ts = best.get('ts', time.time())
                reproj = best.get('reproj_err', None)
                if reproj is not None:
                    sigma = max(0.002*reproj,0.015)
                ...
                if pred is not None:
                    ...
                    # compute robot center from tag4/tag5 if available
                    tag_info = {}
                    ...
                    # fallback: take any world_pos available
                    ...
                    # compute required speed to reach predicted point in tau
                    v_req = None
                    can_reach = None
                    ...
                    # share minimal in-memory info for other modules
                    try:
                        with detect_lock:
                    ...
                    print(f"[EKF PRED] t+{tau:.3f}s -> {p_future}  can_reach={can_reach} v_req={v_req}")
                else:
